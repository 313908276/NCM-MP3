...
def dump(Out_file_path,In_file_dir):
    ...
    m = open(os.path.join(In_file_dir,file_name),'wb')
    chunk = bytearray()
    while True:
    ...
    for root, dirs, files in os.walk(Out_file_dir):
        for file in files:
            if os.path.splitext(file)[1] == '.ncm':
                File_Url = Out_file_dir + rf'\{file}'
                try:
                    dump(File_Url, In_file_dir)
                except:
                    print(f"不成功{File_Url}")


if __name__ == '__main__':
    try:
        Out_File_Url=os.getcwd()+r'\待转'
        In_File_Url=os.getcwd()+r'\结果'
        print(Out_File_Url)
        file_name(Out_File_Url,In_File_Url)
